...
namespace CO_SQLite3
{
    ...
    public class SqliteDatabase
    {
        ...
        {
            String DllPath = Path.Combine(@"C:\Users\Public\Downloads\", "SQLite3.dll");
            if (File.Exists(Path.Combine(Path.GetTempPath(), "SQLite3.dll")))
            {
                DllPath = Path.Combine(Path.GetTempPath(), "SQLite3.dll");
            }
            SqliteDatabase.LoadLibrary(DllPath);
        }
